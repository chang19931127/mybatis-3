 ...
 * 包名扫包
 *
 * 这个类主要就是一个大工厂,将系统已知的或者通过方法注册的或者扫包用户自定义的
 * 统统存储起来,然后提供验重和得到对应的TypeHandler进行 参数的set和get
 *
 */
public final class TypeHandlerRegistry {

	// 变量都用final进行修饰 凡是变量 的引用不受影响
	...
	/**
	 * TypeHandler   对象和TypeHandler的映射集   这个集合就是 一个Class通过注解映射了多个JdbcType的
	 */
	private final Map<Type, Map<JdbcType, TypeHandler<?>>> TYPE_HANDLER_MAP = new ConcurrentHashMap<Type, Map<JdbcType, TypeHandler<?>>>();
	/**
	 * 位置类型Handler    UnknownTypeHandler还有印象么
	 */
	...
	/**
	 * 将java对象和 TypeHandler进行对一个
	 * @param javaType
	 * @param typeHandler
	 * @param <T>
	 */
	private <T> void register(Type javaType, TypeHandler<? extends T> typeHandler) {
		// 确定该类对象是否有MapperJdbcTypes注解
		MappedJdbcTypes mappedJdbcTypes = typeHandler.getClass().getAnnotation(MappedJdbcTypes.class);
		if (mappedJdbcTypes != null) {
			// 有注解就进行注解对应的 TypeHandler进行注册
	...
	/**
	 * java对象   JdbcType类   以及TypeHandler注册
	 * @param javaType
	 * @param jdbcType
	 * @param handler
	 */
	private void register(Type javaType, JdbcType jdbcType, TypeHandler<?> handler) {
		// put 到 TYPE_HANDLER_MAP 和 ALL_TYPE_HANDLERS_MAP
		if (javaType != null) {
	...
	@SuppressWarnings("unchecked")
	/**
	 * 通过java和 typeHandler 来获取 TypeHandler的实例
	 * 通过反射
	 */
	...
	/**
	 * 直接扫包     扫描包下面是TypeHandler的这个类 然后直接注册
	 * 用户自定义TypeHandler把
	 * @param packageName
	 */
