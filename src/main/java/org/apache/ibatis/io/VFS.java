 ...
 *
 * @author Ben Gunter
 * 1. mybatis自己实现了日志的门面,并在资源加载的时候进行了日志的输出
 * 2. VFS 翻译过来就是 very simple for accessing resources within an application server
 * 为应用服务器提供很简单的资源访问
 *
 * 抽象父类
 * 一种是默认VFS 一种是JBoss6VFS    去看看他们之间的区别把
 *
 * 四个子类可以调用的方法,没有抽出来工具类,加载类,加载资源,反射获得Method,反射invoke Method
 *
 * 提供两个个抽象方法 子类自定义实现
 * isValid                          判断是否有效,因为是自定义的VFS 肯定在某种条件下才会生效
 * list(URL url, String forPath)    通过url 到 forpath的资源    结果就是url中找到匹配forPath的资源
 *                                  example  url 是src/main/java    forPath 是 src/main    就能找出url 下的资源
 *                                  在url中找满足forPath的资源
 *
 * VFS提供了简单的获得资源的方式,所有资源.以及可以去反射调用加载的类,等等
 *
 */
public abstract class VFS {
	...
	private static class VFSHolder {
		...
		static VFS createVFS() {
			...
			VFS vfs = null;
			// 遍历直到 vfs!=null 并且vfs.isValid有效
			for (int i = 0; vfs == null || !vfs.isValid(); i++) {
				Class<? extends VFS> impl = impls.get(i);
				try {
					vfs = impl.newInstance();
					// vfs null 或者 vfs 无效 打日志告知
					if (vfs == null || !vfs.isValid()) {
						// 注意这种日志的打印方式,针对debug的日志这么操作info不需要
	...
	public static VFS getInstance() {
		// 通过静态内部类的static方法,被调用放调用就加载,来实现懒加载
	 ...
	 * 用户自定义VFS 然后自己通过这个方法加载进来,存放在父类变量中
...
//      return ReflectUtil.findClass(className);
	...
	/**
	 * 提供三个抽象方法
	 * 一个是判断是否有效,因为是自定义的VFS 肯定在某种条件下才会生效
	 * 一个是
	 *
	 */
	 ...
	 * 递归找出 url下面的所有资源,这些资源forPath子目录的
	 * 这个方法可以被递归调用    一个路径下的URL下面的URL 都要调用 但是forPath不变
