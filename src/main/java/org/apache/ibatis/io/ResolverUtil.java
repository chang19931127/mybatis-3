 ...
 * </pre>
 * 一个资源发现,鉴别的工具类
 * 通过内部类来进行matchs来进行操作 是否属于一个类，是否有注解
 * 两个功能 查找包下的匹配内容
 *      1. 包下面的继承或者实现Class的类
 *      2. 包下面具有某个Annotation的类
 * 这个工具的作用就是扫包了！！
 *
 * @author Tim Fennell
 */
public class ResolverUtil<T> {
	...
	public ClassLoader getClassLoader() {
		// 线程上下文类加载器,这个类加载器是否有特殊之处,记得之前,这个类加载器可以实现热部署
	 ...
	 * {@link #getClasses()}.
	 *
	 * 通过包名,和类比较来进行包下面的相关类的查找
	 * 这种操作就可以扫包,来获取某些继承啊实现啊的类
	 *
	 ...
	 * 匹配方法,匹配成功并加到matches这个hashSet中存放
