...
/**
 * 主要使用了 mybatis 的默认配置
 * 测试了 @AutomapConstructor 注解的作用 会指定使用注解的构造器
 */
public class AutoConstructorTest {

	// SqlSessionFactory 对象最好使用单例
	...
	public void annotatedSubject() {
		final SqlSession sqlSession = sqlSessionFactory.openSession();
		try {
			// 使用 @AutomapConstructor 来指定自动映射的 构造器 这个注解可以有很多个 会有一个默认的排序
			// 这个默认排序和编译后的字节码 获取的 Constructs 数组的顺序有关
			final AutoConstructorMapper mapper = sqlSession.getMapper(AutoConstructorMapper.class);
